...
# WIFI_SSID = "PitmanStudy"  # "msg.network"
# WIFI_KEY = "0826583900"  # "msg.t66Yu9"

# HOST_IP = "192.168.68.103"  # change based on new wifi PC: 192.168.0.32
# HOST_PORT = 5005

# LAB Colour Thresholds -> Calibrate
redThreshold = (0, 100, 12, 127, -7, 81)
greenThreshold = (4, 100, -128, -18, -128, 127)
blueThreshold = (0, 67, -128, 17, -128, -8)
yellowThreshold = (51, 100, -128, 56, 29, 127)

thresholds = [redThreshold, greenThreshold, blueThreshold, yellowThreshold]
num_features = len(thresholds)

colour_labels = ["RED", "GREEN", "BLUE", "YELLOW"]
draw_colours = [(255, 255, 255), (255, 255, 255), (255, 255, 255), (0, 0, 0)]
...
while True:
    ...
    for i in range(num_features):
        ...
        col = draw_colours[i]
        label = colour_labels[i]
        # Draw cross
        img.draw_line(cx - cross_size, cy, cx + cross_size, cy, color=col, thickness=2)
        img.draw_line(cx, cy - cross_size, cx, cy + cross_size, color=col, thickness=2)
        # Bounding box
        bx = cx-w//2
        by = cy-h//2
        img.draw_rectangle(bx, by, w, h, color=col, thickness=1)
        # Label
        ty = max(by-12, 0)
        img.draw_string(bx, ty, label, color=col, scale=1, mono_space=False)
